...
namespace _2.BunnyFactory
{
    class BunnyFactory
    {
        ...
        static ulong CurrentSum(List<int> cages, int i, int nextcages)
        {
            ulong curSum = 0;

            for (int next = i + 1; next <= i + nextcages; next++)
            {
                curSum += (ulong) cages[next];
            }

            return curSum;
        }
        static ulong CurrentProduct(List<int> cages, int i, int nextcages)
        {
            ulong curProduct = 1;

            for (int next = i + 1; next <= i + nextcages; next++)
            {
                curProduct *= (ulong)cages[next];
            }
        ...
        static void Main()
        {
            ...
            for (int i = 0; i < cages.Count; i++)
            {
                ...
                cages.Insert(0, (int)currentProduct);
                cages.Insert(0, (int)currentSum);
